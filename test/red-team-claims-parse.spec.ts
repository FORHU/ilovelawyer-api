...
import { extractRedTeamClaims } from "../src/utils/red-team-claims-parse";

describe("extractRedTeamClaims", () => {
  it("returns undefined when there is no [CLAIMS] block at all", () => {
    expect(extractRedTeamClaims("just a plain assessment with no tags")).to.equal(undefined);
  });

  it("returns an empty array when the model explicitly found nothing to flag", () => {
    expect(extractRedTeamClaims("[CLAIMS]\n[]\n[/CLAIMS]")).to.deep.equal([]);
  });

  it("parses a well-formed closed block", () => {
    const text = `[CLAIMS]
[{"text": "The contract lacks a signature.", "category": "GROUNDED", "sourceLabel": "No signed contract on file"}]
[/CLAIMS]`;
    expect(extractRedTeamClaims(text)).to.deep.equal([
      { text: "The contract lacks a signature.", category: "GROUNDED", sourceLabel: "No signed contract on file" },
    ]);
  });
  ...
  it("falls back to an open tag when the closing tag is missing (streaming cutoff)", () => {
    ...
    const result = extractRedTeamClaims(text);
    expect(result).to.have.length(1);
    expect(result![0].text).to.equal("Some claim");
  });

  it("drops rows with no text — nothing to match back onto the assessment", () => {
  ...
  it("caps at 30 items even when the model returns more", () => {
    ...
    expect(extractRedTeamClaims(text)).to.have.length(30);
  });
