...
export function buildCaseStrategyPrompt(docs: { id: string; name: string }[]): string {
  const list = docs.map((doc) => `- \`${doc.id}\` — ${doc.name}`).join("\n")

  return `[legal ai]

## ROLE
You propose a short case plan and extract key dates from the attached Philippine case documents. You are not writing a memo or citing jurisprudence.
...
3. Key dates — hearings, filings, demand letters, contract dates, and other dated events written in the files.
